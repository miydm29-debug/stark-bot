...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = await update.message.reply_text("🔍 جاري جلب البيانات بدقة...")
    ...
    try:
        ...
        if not redirect_match:
            await msg.edit_text("❌ لم يتم العثور على رابط التحويل.")
            return

        redirect_url = redirect_match.group(1).replace(r'\u0026', '&')
        
        # بنطلب الصفحة النهائية اللي فيها الفعالية
        res_redirect = requests.get(redirect_url, cookies=COOKIES_DICT, headers=headers, timeout=15)
        final_page = res_redirect.text

        # طباعة جزء من المحتوى في لوجز ريلواي عشان نشوف هيكل الصفحة بوضوح
        print("--- PAGE SNIPPET ---", final_page[:500])

        # بحث صارم عن الأيدي (أرقام سداسية أو سباعية أو ثمانية أو تسعة مميزة لألعاب ببجي) وتجنب المتغيرات
        player_id = "غير محدد"
        # نبحث عن الكلمات المفتاحية الحقيقية للأيدي داخل الـ JSON
        id_match = re.search(r'["\'](?:roleId|playerId|userId|uin|openId)["\']\s*:\s*["\']?(\d{8,12})["\']?', final_page)
        if id_match:
            player_id = id_match.group(1)

        player_name = "غير معروف"
        name_match = re.search(r'["\'](?:nickName|roleName|userName|playerName)["\']\s*:\s*["\']([^"\']+)["\']', final_page)
        if name_match and "mp_" not in name_match.group(1) and "muid" not in name_match.group(1):
            player_name = name_match.group(1)

        elapsed_time = round(time.time() - start_time, 1)

        await msg.edit_text(
            f"🛡️ <b>Midasbuy Bot</b>\n\n"
