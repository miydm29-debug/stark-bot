...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # بحث أدق عن اسم اللاعب وتجنب أسماء المتغيرات البرمجية
        player_name = "غير معروف"
        name_patterns = [
            r'["\'](?:nickName|playerName|roleName|userName)["\']\s*:\s*["\']([^"\']+)["\']',
            r'["\']name["\']\s*:\s*["\']([^"\']{3,20})["\']'
        ]
        
        for pattern in name_patterns:
            matches = re.findall(pattern, final_page)
            for name in matches:
                if name and not name.startswith("mp_") and not "pool" in name:
                    player_name = name
                    break
            if player_name != "غير معروف":
                break

        # بحث عن الأيدي الحقيقي (أرقام تتراوح بين 9 و 11 رقم وتكون مرتبطة بـ roleId أو openId أو playerId)
        player_id = "غير محدد"
        id_match = re.search(r'["\'](?:roleId|openId|playerId|accountId|uid)["\']\s*:\s*["\']?(\d{9,11})["\']?', final_page)
        if id_match:
            player_id = id_match.group(1)
        else:
            # لو مش موجودة صراحة، ندور على أي رقم بيبدأ بـ 5 أو 6 أو 7 وطوله مظبوط
            fallback_id = re.search(r'\b([5-7]\d{8,10})\b', final_page)
            if fallback_id:
                player_id = fallback_id.group(1)
